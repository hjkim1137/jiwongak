...
const PERSONALITY_AXIS_KO: Record<string, string> = {
  stress_tolerance: "스트레스 내성",
  sensitivity: "민감도",
  change_adaptability: "변화 적응성",
};

const PERSONALITY_POLE_KO: Record<string, string> = {
  high: "high",
  low: "low",
  balanced: "balanced",
};
...
const SCORE_TOOL: Anthropic.Tool = {
  ...
  input_schema: {
    type: "object" as const,
    required: ["skill_match", "wlb", "career_ceiling", "personality_fit"],
    properties: {
      skill_match: dimensionSchema("스킬 매칭"),
      wlb: dimensionSchema("워라밸"),
      career_ceiling: dimensionSchema("커리어 천장"),
      personality_fit: dimensionSchema("성격 적합도"),
    },
  },
};
...
function buildProfilePrompt(profile: UserProfile): string {
  ...
  const personalityBlock = profile.personality_profile
    ? Object.entries(profile.personality_profile.classification)
        .map(([axis, pole]) => {
          const score = profile.personality_profile!.axisScores[
            axis as keyof typeof profile.personality_profile.axisScores
          ];
          return `- ${PERSONALITY_AXIS_KO[axis] ?? axis}: ${PERSONALITY_POLE_KO[pole] ?? pole} (high=${score.high}, low=${score.low})`;
        })
        .join("\n")
    : "(성격 진단 미수행 — personality_fit confidence 0으로 강제)";
...
4. personality_fit: 사용자의 3축 성격이 공고가 시사하는 환경에 얼마나 부합하는지 채점한다.
   - 근거는 공고의 cultural_keywords + culture_pattern 인사이트만 사용. 둘 다 비면 confidence ≤ 0.4
   - **양극 모두 강점 프레임**: high가 좋고 low가 나쁜 것이 아니다. "환경 요구"와 사용자 극의 일치도가 핵심
   - 예시 매칭: 빠른 의사결정·잦은 피벗 환경 → stress_tolerance high·change_adaptability high에 유리 / 디테일 중시·안정된 프로세스 환경 → sensitivity high·change_adaptability low에 유리
   - balanced 축은 어느 환경에서도 중립 — 큰 가감 없음
5. **evidence 형식 필수**: 인사이트를 근거로 쓸 때 반드시 slug를 그대로 넣어라. 예: "robotics-frontend-not-core". 스킬을 근거로 쓸 때는 스킬명. 설명문 금지, slug/스킬명만
6. 같은 입력에 대해 항상 같은 점수 (결정적)
7. severity=critical 인사이트가 있으면: (a) 해당 dimension의 evidence에 해당 slug 필수 포함 (b) flags에 경고 필수 포함
8. **flags 규칙**: 공고·산업·직무에서 발견된 구체적 위험 신호만 넣는다. "신호 없음", "근거 부족", "불확실", "판단 어려움", "스킬 미입력", "스킬 없음", "매칭 불가" 같은 불확실성·데이터 부재 메모는 flags에 절대 넣지 않는다 — 불확실성은 confidence 값(0~1)으로만 표현한다`;
}
...
문화 신호: ${posting.raw_signals?.cultural_keywords?.join(", ") || "없음"}
...
export async function scoreDimensions(
  ...
  // 성격 진단 미수행 사용자 → personality_fit 가중평균 기여 0
  // (composeResult가 confidence × weight로 정규화하므로 안전하게 무력화)
  if (!profile.personality_profile) {
    const idx = result.findIndex((d) => d.dimension === "personality_fit");
    if (idx !== -1) {
      result[idx] = {
        dimension: "personality_fit",
        score: 50,
        confidence: 0,
        evidence: [],
        flags: [],
  ...
  // 공고에서 cultural_keywords 추출이 비면 personality_fit confidence를 0.4 이하로 하향
  // (Claude가 근거 없이 추측한 점수가 가중평균을 흔드는 것을 방지)
  const culturalKeywords = posting.raw_signals?.cultural_keywords ?? [];
  if (culturalKeywords.length === 0) {
    const idx = result.findIndex((d) => d.dimension === "personality_fit");
    if (idx !== -1 && result[idx].confidence > 0.4) {
      result[idx] = { ...result[idx], confidence: 0.4 };
    }
  }
