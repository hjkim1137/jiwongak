...
> **이 공고 지원각이야?** — 채용공고를 붙여넣으면 나의 경력·역량·라이프스타일과 매칭해 지원 적합도를 한 마디로 알려주는 AI 서비스
...
구직자는 적합도를 모른 채 무차별 지원한다. 기존 매칭 서비스(원티드 AI매칭 등)는 키워드 매칭 위주이고, **산업·직무의 구조적 특성(커리어 천장, 락인 리스크, 희소성)을 알려주지 않는다.**

지원각은 단순 적합도뿐 아니라 "이 공고는 SI 계열이라 이직 시장 가치가 낮다" 같은 **구조적 인사이트**를 함께 제공한다.
...
강제 양자택일 8문항으로 6개 타입(N잡 라이프러·성장형 도전자·점프업 야심가·안정형 워라밸러·창업가형·균형 추구자) 중 하나를 자동 분류한다.
사용자는 **가중치 슬라이더를 건드리지 않는다** — 타입별로 사전 정의된 가중치 프리셋(skill/wlb/career_ceiling)이 자동 적용된다.
...
### 3. 근거 인사이트 인용 (RAG)
분석 결과에 **"이 분석의 근거"** 섹션으로 인용된 인사이트와 severity(참고/주의/위험)를 표시한다. 환각 방지를 위해 직접 큐레이션한 74개 인사이트 DB에서만 근거를 가져온다.
...
"사용자는 본인의 정확한 가중치를 모른다"는 가설. 8문항 양자택일 진단으로 라이프스타일 타입을 분류하고, 타입별 사전 정의 가중치를 자동 적용한다. 사용자는 가중치 존재 자체를 모른다.

### LLM 채점 ↔ 결정적 코드 명확히 분리
- **LLM**: dimension별 점수+근거 추출만
- **코드**: 가중치 적용, 라벨 결정, 함정각 오버라이드

→ eval set으로 회귀 테스트 가능, 단위 테스트 가능, 같은 입력에 항상 같은 결과

### 환각 방지: 큐레이션 인사이트 DB + RAG 강제
블라인드 크롤링이나 LLM 자유 생성 없이, 직접 큐레이션한 74개 인사이트에서만 근거를 인용하도록 프롬프트로 강제한다. 근거가 없으면 confidence를 낮춰 종합 점수에 영향을 줄인다.

### Prompt Caching으로 비용 절감
같은 사용자가 여러 공고를 분석할 때 system 프롬프트(사용자 프로필 + 채점 규칙)를 캐싱한다. 공고당 채점 비용 **~70% 절감**.
...
"제약영업 인사이트가 보험 공고에 인용되는" 버그를 early return 설계로 완전 차단했다.

---

## Eval 결과

LLM 파이프라인의 점수 일관성을 검증하기 위해 10개 공고에 직접 gold label을 부여하고 회귀 테스트를 구축했다.
...
**튜닝 포인트**: `career_ceiling` 채점 기준을 "산업 성장성"에서 **"본인 커리어 이직 시장 가치"**로 명확화 + 직군 미스매치 페널티(0~15) + 니치 산업 페널티(≤55) + 산업 매력도 평가 금지 조항 추가.
...
필요한 환경변수 목록은 `.env.local.example` 참고.
